...
main_with_controler() ->
    S = self(),
    spawn(fun() -> client1(S) end),
    receive PidA -> PidA end,
    spawn(fun() -> client2(PidA, S) end),
    receive PidB -> PidB end,
    timer:sleep(2000),
    delete_with_controller(PidA, PidB).
...
delete_with_controller(PidA, PidB) ->
    io:format("***Items before deleting:~n", []),
    io:format("Carl:~n", []),
    trade_fsm:show_items(PidA),
    timer:sleep(500),
    io:format("Jim:~n", []),
    trade_fsm:show_items(PidB),
    timer:sleep(500),
    trade_fsm:delete_item(PidB, "shotgunlllllllllll"),
    timer:sleep(1500),
    io:format("***Items after deleting:~n", []),
    io:format("Carl:~n", []),
    trade_fsm:show_items(PidA),
    timer:sleep(500),
    io:format("Jim:~n", []),
...
main_ab() ->
    ...
    receive PidA -> PidA end,
    spawn(fun() -> b(PidA, PidCliA, S) end),
    receive PidB -> PidB end,
    delete_with_controller(PidA, PidB).
...
a(Parent) ->
    {ok, Pid} = trade_fsm:start_link("Carl"),
    Parent ! Pid,
    io:format("PID from start_link: ~p~n", [Pid]),
    io:format("Spawned Carl: ~p~n", [Pid]),
    %sys:trace(Pid,true),
    timer:sleep(800),
    trade_fsm:accept_trade(Pid),
