...
show_items(OwnPid) ->
    gen_fsm:send_event(OwnPid, {show_items}).

delete_item(OwnPid, Item) ->
    gen_fsm:send_event(OwnPid, {delete_item, Item}).
...
negotiate({show_items}, S=#state{}) ->
    io:format("Own items: ~p~n", [S#state.ownitems]),
    io:format("Other items: ~p~n", [S#state.otheritems]),
    {next_state, negotiate, S#state{}};

negotiate({delete_item, Item}, S=#state{}) ->
    IsMemberOfOwnitems = lists:member(Item, S#state.ownitems),
    IsMemberOfOtheritems = lists:member(Item, S#state.otheritems),
    CanDelete = IsMemberOfOtheritems or IsMemberOfOwnitems,
    if 
        CanDelete -> 
                    notice(S, "Delete item: ~p", [Item]),
                    % undo_offer(S#state.other, Item),
                    OwnItems = S#state.ownitems,
                    OtherItems = S#state.otheritems,
                        % {next_state, negotiate, S#state{otheritems=remove(Item, OtherItems)}};
                    {next_state, negotiate, S#state{ownitems=remove(Item, OwnItems), otheritems=remove(Item, OtherItems)}};
        true -> 
                    notice(S, "Cannot delete item: ~p", [Item]),
                    {next_state, negotiate, S#state{}}
    end;
